...
function buildBrew() {
  ...
  ctx = gsap.context(() => {
    ...
    /* One start time per row, shared by its dot, its line and its label, so the
       text rides in behind its own line's leading edge instead of arriving
       after every line has already finished. Without this the middle of the
       phase reads as hairlines pointing at nothing. */
    const rowAt = (i) => lbA + i * TUNING.labelStagger * lbSpan * 0.9;

    connectors.forEach((c, i) => {
      tl.to(c.dot,  { scale: 1, duration: lbSpan * 0.08 }, rowAt(i));
      tl.to(c.line, { strokeDashoffset: 0, duration: lbSpan * 0.30, ease: 'power2.out' }, rowAt(i) + lbSpan * 0.03);
    });

    el.labels.forEach((lbl, i) => {
      // 0.16 in: the line is ~half drawn, so text and rule land together
      const at = rowAt(i) + lbSpan * 0.16;
      ...
      tl.fromTo(lbl, { opacity: 0 }, { opacity: 1, duration: lbSpan * 0.24, ease: 'power2.out' }, at);
      tl.fromTo(lbl.children,
        { x: L.mobile ? 0 : dirSign() * 14, y: L.mobile ? 12 : 0 },
        { x: 0, y: 0, duration: lbSpan * 0.24, ease: 'power2.out' },
        at);
    });
